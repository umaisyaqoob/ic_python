...
def max_return_for_volatility(returns, max_volatility):
    num_assets = len(returns.columns)
    args = (returns,)
    
    constraints = [
        {'type': 'eq', 'fun': lambda x: np.sum(x) - 1},  # Sum of weights = 1
        {'type': 'ineq', 'fun': lambda x: max_volatility - portfolio_volatility(x, returns)}  # Portfolio volatility <= max_volatility
    ]
    
    bounds = tuple((0, 1) for asset in range(num_assets))
    initial_guess = [1./num_assets] * num_assets
    
    result = minimize(neg_portfolio_return, initial_guess, args=args, method='SLSQP', bounds=bounds, constraints=constraints)
    return result.x
...
max_volatility = 0.15  # Example maximum volatility

# Calculate optimized weights
optimized_weights_min_vol_target_return = min_volatility_for_target_return(returns, target_return)
optimized_weights_max_return_for_vol = max_return_for_volatility(returns, max_volatility)

# Convert optimized weights to pandas Series (if you're working with single-period optimization)
optimized_weights_series_min_vol_target_return = pd.Series(optimized_weights_min_vol_target_return, index=returns.columns)
optimized_weights_series_max_return_for_vol = pd.Series(optimized_weights_max_return_for_vol, index=returns.columns)

# If your optimizations are supposed to apply to a single date or you're applying the same weights across multiple dates,
# you can directly multiply the weights Series by the returns DataFrame and then sum across rows to get portfolio returns:
optimized_portfolio_daily_returns_min_vol_target_return = (returns * optimized_weights_series_min_vol_target_return).sum(axis=1)
optimized_portfolio_daily_returns_max_return_for_vol = (returns * optimized_weights_series_max_return_for_vol).sum(axis=1)

# Calculate cumulative returns for each strategy
cumulative_returns_max_sharpe = (1 + optimized_portfolio_daily_returns).cumprod()
cumulative_returns_min_vol = (1 + optimized_portfolio_daily_returns_min_vol).cumprod()
cumulative_returns_max_return = (1 + optimized_portfolio_daily_returns_max_return).cumprod()
cumulative_returns_min_vol_target_return = (1 + optimized_portfolio_daily_returns_min_vol_target_return).cumprod()
cumulative_returns_max_return_for_vol = (1 + optimized_portfolio_daily_returns_max_return_for_vol).cumprod()

# Plotting
plt.figure(figsize=(14, 7))

plt.plot(cumulative_returns_max_sharpe, label='Max Sharpe Ratio')
plt.plot(cumulative_returns_min_vol, label='Min Volatility')
plt.plot(cumulative_returns_max_return, label='Max Return')
plt.plot(cumulative_returns_min_vol_target_return, label='Min Volatility for Target Return')
plt.plot(cumulative_returns_max_return_for_vol, label='Max Return for Volatility')
...
ALPACA_API_KEY = 'PKC5U9TOM9VLUAG6D918'
ALPACA_SECRET_KEY = 'JWPsdGwKEhe1Jsugv8qMx43QCcV8IrqMbwyJUKWZ'

# Initialize the Alpaca API for paper trading
api = tradeapi.REST(ALPACA_API_KEY, ALPACA_SECRET_KEY, base_url='https://paper-api.alpaca.markets')  # Change to live URL for real trading
...
def optimize_portfolio(returns):
    ...
    bounds = tuple((0, 1) for asset in range(num_assets))
    initial_guess = [1./num_assets] * num_assets
    opt_results = minimize(neg_sharpe_ratio, initial_guess, args=args,
                           method='SLSQP', bounds=bounds, constraints=constraints)
    return opt_results.x

def is_nyse_open():
    now = datetime.now(pytz.timezone('America/New_York'))  # Corrected
    open_time = now.replace(hour=9, minute=30, second=0, microsecond=0)
    close_time = now.replace(hour=16, minute=0, second=0, microsecond=0)
    return open_time <= now <= close_time

def wait_until_market_opens():
    while not is_nyse_open():
        now = datetime.datetime.now(pytz.timezone('America/New_York'))
        next_open = now.replace(hour=9, minute=30, second=0, microsecond=0) if now.hour < 16 else now.replace(day=now.day+1, hour=9, minute=30, second=0, microsecond=0)
        wait_seconds = (next_open - now).total_seconds()
        print(f"Market is closed. Next opening at {next_open.strftime('%Y-%m-%d %H:%M:%S')}. Waiting for {int(wait_seconds / 60)} minutes.")
        time.sleep(max(wait_seconds, 60))  # Wait until market opens or check every minute

def is_rebalance_week():
    """
    Determines if the current week is one of the weeks to perform rebalancing.
    This function assumes rebalancing starts from the first Monday of the year
    and occurs bi-weekly thereafter. Adjust the logic if starting on a different date.
    """
    # Calculate the number of weeks since the first Monday of the year
    first_monday = datetime(datetime.now().year, 1, 1)  # This line is corrected
    while first_monday.weekday() != 0:  # 0 is Monday
        first_monday += timedelta(days=1)  # Use 'timedelta' directly

    current_date = datetime.now()  # Use 'datetime.now()' directly
    weeks_since_first_monday = (current_date - first_monday).days // 7

    # Return True if the current week is an even week since the first Monday of the year
    return weeks_since_first_monday % 2 == 0

def rebalance_portfolio(returns, tickers, target_allocations):
    try:
        # Fetch current allocations and account information
        current_positions = api.list_positions()
        account_info = api.get_account()
        account_equity = float(account_info.equity)
        buying_power = float(account_info.buying_power)  # Use buying power for adjustments

        current_allocations = {
            position.symbol: (float(position.qty) * float(position.current_price)) / account_equity
            for position in current_positions if position.symbol in tickers
        }

        # Determine if rebalancing is necessary
        for symbol, target_allocation in target_allocations.items():
            current_value = current_allocations.get(symbol, 0.0) * account_equity
            desired_value = target_allocation * buying_power
            price = api.get_latest_trade(symbol).price
            quantity_to_adjust = (desired_value - current_value) / price

            # Execute trades
            if quantity_to_adjust > 0:
                # Buy or increase position
                print(f"Buying {quantity_to_adjust} of {symbol}")
                api.submit_order(
                    symbol=symbol,
                    qty=abs(quantity_to_adjust),  # Use abs() for quantity to ensure it's positive
                    side='buy',
                    type='market',
                    time_in_force='day',
                    order_class='simple',
                    fractional=True  # Specify fractional=True for fractional shares
                )
            elif quantity_to_adjust < 0:
                # Sell or short position
                current_position_qty = float(next((item.qty for item in current_positions if item.symbol == symbol), 0))
                if current_position_qty > 0:
                    # If currently holding the stock, sell to reduce position
                    print(f"Selling {abs(quantity_to_adjust)} of {symbol}")
                    api.submit_order(
                        symbol=symbol,
                        qty=abs(quantity_to_adjust),  # Sell part of the current position
                        side='sell',
                        type='market',
                        time_in_force='day',
                        order_class='simple',
                    ...
                    # If not holding the stock, short selling
                    print(f"Short selling {abs(quantity_to_adjust)} of {symbol}")
                    api.submit_order(
                        symbol=symbol,
                        qty=abs(quantity_to_adjust),  # Use abs() to ensure positive quantity for short selling
                        side='sell',
                        type='market',
                        time_in_force='day',
                        order_class='simple',
        ...
        print(f"An error occurred while rebalancing: {e}")

def calculate_target_allocations(optimized_weights):
    """
    Extracts the last row of the optimized weights DataFrame to use as the target allocations.

    Parameters:
    - optimized_weights: DataFrame containing the optimized weights for each ticker over time.

    Returns:
    - Dictionary with ticker symbols as keys and target allocation percentages as values.
    """
    last_allocations = optimized_weights.iloc[-1]
    target_allocations = last_allocations.to_dict()
    return target_allocations

# Example usage:
# Make sure to call this after performing the optimization and just before rebalancing
target_allocations = calculate_target_allocations(optimized_weights_max_return)
...
def is_rebalance_week():
    ...
    """
    # Calculate the number of weeks since the first Monday of the year
    first_monday = datetime(datetime.now().year, 1, 1)
    while first_monday.weekday() != 0:  # 0 is Monday
...
def scheduled_rebalance():
    if is_rebalance_week() and is_nyse_open():
        print("Market is open and it's the scheduled week. Proceeding with rebalancing...")
        # Your rebalancing logic here
    elif not is_nyse_open():
        print("Market is closed. Waiting for market to open for rebalancing.")
        wait_until_market_opens()
        # Make sure to check is_rebalance_week() again after waiting
        if is_rebalance_week():
            # Your rebalancing logic here
            pass
    else:
        print("It's not a rebalancing week.")

# Schedule to check every Monday at 10:00 AM
schedule.every().monday.at("15:30").do(scheduled_rebalance)
